...
if [ "${UNIT_DEBUG}" == 1 ] || [ "${UNIT_DEBUG}" == 'TRUE' ] ||  [ "${UNIT_DEBUG}" == 'true' ] || [ "${UNIT_DEBUG}" == 'True' ]; then
    cp /usr/lib/unit-debug/sbin/*    /usr/lib/unit/sbin/
    cp /usr/lib/unit-debug/modules/*  /usr/lib/unit/modules/
fi
...
if [ "$1" = "/usr/lib/unit/sbin/unitd" ]; then
    if /usr/bin/find "/usr/lib/unit/state/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        echo "$0: /var/lib/unit/state/ is not empty, skipping initial configuration..."
    else
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            echo "$0: /docker-entrypoint.d/ is not empty, launching Unit daemon to perform initial configuration..."
            /usr/lib/unit/sbin/unitd --control unix:/var/run/control.unit.sock --pid /var/run/unit.pid --modules /usr/lib/unit/modules --state /usr/lib/unit/state --log /var/log/unit.log --tmp /var/tmp/
